...
* Some IDE plugins have problems with the default JVM options in Payara Server 5.184. See the Known issues section in case you run into problems
...
* Fresh Payara Server 5.184 doesn't start in Netbeans. This is because the Netbeans plugin doesn't understand Java version selectors in JVM options. As a workaround, replace multiple JVM options for `grizzly-npn-bootstrap` in the https://github.com/payara/Payara/pull/3414/files#diff-106a22dc9db673bf7b67c76cf69914aaR256[default domain.xml] with a single option that matches your Java version and remove the Java version selector. The same workaround my be necessary for other IDEs like Eclipse or IntelliJ.