...
With the introduction of the link:/documentation/payara-server/hazelcast/README.adoc[Domain Data Grid]
...
which can be useful due to the variable networking inherent to kubernetes. For this clustering mode,
you must specify the kubernetes namespace and service name separated by a comma - Payara Micro will then attempt
to cluster with instances present on pods within the specified namespace and service.
