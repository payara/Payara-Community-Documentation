...
(with different HTTP port configurations), and you should see the following in the
log output:
...
Besides the multicast discovery, Hazelcast supports other forms of discovery, e.g.
by specifying a range of IP addresses (via the *TCP joiner*). These forms of
discovery are not configurable directly from Payara Micro, but can be configured
using a standard Hazelcast configuration file provided with the `--hzConfigFile`
