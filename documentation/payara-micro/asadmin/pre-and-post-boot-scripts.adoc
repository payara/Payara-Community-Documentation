...
* Each asadmin command may contain references to environment variables or system properties as described in link:/documentation/payara-server/server-configuration/var-substitution/usage-of-variables.adoc[Using variables]
 (__Since Payara Micro 4.1.2.174)
...
When running a Payara Micro from the command line, use appropriate command file options to specify the script file locations and the moment in the boot lifecycle when they should be executed:
