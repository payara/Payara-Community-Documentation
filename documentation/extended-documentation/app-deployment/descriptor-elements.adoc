...
For more detail, see the link:/documentation/extended-documentation/ejb.adoc[Enhanced EJB configuration] section.
...
*EJBWS1* and *EJBWS*, and they need to be secured using BASIC authentication against
the _file_ security realm, the following configuration would do it:
...
<glassfish-ejb-jar>
  <ejb>
    ...
    <webservice-endpoint>
       <port-component-name>EJBWS1Post</port-component-name>
  ...
  <ejb>
    ...
    <webservice-endpoint>
       <port-component-name>EJBWS2Post</port-component-name>
...
for each EJB component, then the Web service definition information would be repeated
too (Port Component Name, Endpoint Address, etc.), which would be too cumbersome
for cases when there are lots of EJB Web services.
...
inside it will apply to all of the EJB defined Web Services inside the assembly.
...
NOTE: All sub-elements of the `login-config` can be used on the `default-login-config`
without issues.
