...
|`uberJarClassifier`
|microbundle
|Specifies the classifier to use for the generated uber-jar.
...
<plugin>
    ...
    <configuration>
        ...
        <payaraVersion>{{ book.currentVersion }}</payaraVersion>
        <artifactItem>
...
|`payaraVersion`
|{{ book.currentVersion }}
|By default start mojo fetches payara-micro with version {{ book.currentVersion }}.
...
|`useUberJar`
|false
|Use created uber-jar name that resides in target folder to identify the process id 
of the running payara-micro.

|`uberJarClassifier`
|microbundle
|Specifies the classifier of the generated uber-jar to identify the process id 
of the running payara-micro.
