...
|_none_
|Defines a list of system properties that will be applied before `java` executable.
System properties can either be defined as key-value pairs or just as list of
values separately. key-value pairs will be formatted as `key=value`.
