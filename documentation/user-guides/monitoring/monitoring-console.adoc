...
| Coloring | _Instance Name_ | Sets the property of a metric data series that is used to select the colors the data is shown with. _Instance Name_ will show data in same color that belongs to the same instance. _Series Name_ will show data in same color what is the very same metric series. _Result Set Index_ will show data in same color that is at the same index in the server response.
...
|====================
| Property | Default | Description
| Scheme | - | The selected scheme is applied to set the colors palette for _Data #_ as well as _Defaults_ and _Opacity_. This is only a convinient way to switch all of these between different settings. The individual properties can be changed afterwards as usual without affecting the scheme.
...
| Defaults | (default of _Payara_ scheme) | Sets the default colors for _Waterline_, _Alaraming_ and _Critical_ lines. These can be individually overridden in the widet's configuration for these lines.
| Opacity | 20 | Background opacity for lines or bars in graphs, 0 (fully transparent) to 100 (solid).
|====================
