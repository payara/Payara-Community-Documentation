...
For example to get an alert every time a HTTP request causes a server error (responds with any of the 5xx status codes) a custom watch is installed.
...
Optionally a stop condition can be given. It has to be met to stop the alert once it is started. If no such stop condition is given an alert stops as soon as its start condition is no longer met.
Such pairs of start and stop can be defined for 3 levels:

* Unhealthy: corresponds to a "red" alert
* Degraded: corresponds to a "amber" alert
...
The field after the _If_ holds the name of the metric series the watch is linked to, in this case the `ns:http ServerCount5xx` metric, as shown on the _HTTP_ page. The _in_ clause specifies the unit type of the given metric, here it is a `Count`. Following _is_ the comparison operator is selected, here `>`. This is followed by the threshold value, here `3` or `1`, and the scope of the comparison, here `in sample`.
...
| `If` _metric-series_ | Both |Give the name of the metric series to watch. Use another page with widgets to discover available metrics. Their exact name is shown in the _Data_ Settings for property _Series_.
...
| `is` _scope-length_ | Start | The length defines how many of the most recent data points of the watched metric are included in the scope of the comparison.
| `is` _scope-type_ | Start | Does the scope consist of the most recent number of points ('x') or the point in the most recent duration given in milliseconds ('ms').
| `until` _operator_ | Stop | same as for start
| `until` _scope_ | Stop | same as for start
| `until` _scope-length_ | Stop | same as for start
| `until` _scope-type_ | Stop | same as for start
|====================
...
|====================
| Scope | Description
| `for last` | The comparison must be true for each point in the scope for the condition to be met. Similar to a _all_ logic. The number of points included is specified afterwards as number of duration from which to consider points.
...
Watches that originate from server modules cannot be replaced. To change them disable the existing watch, click to edit it which creates a duplicate with a different name that can be adjusted freely.
...
When changing the name of edited watch to an existing name this overrides that watch. In other words watches are always identified by their name.
...
The current state of each watch is shown by the left symbol:

* a pause symbol means the watch is disabled
* a recoding symbol (circle) means the watch is enabled
* a stop symbol (square) means the watch is stopped (permanently)

The status is also reflected by the checkbox left of the watches name, checked for enabled, unchecked for disabled.
Disabled watches names are also greyed out.
To disable an enabled watch un-check the checkbox or use the _Disable_ item form the cogs menu on the right of the watch widget.
