...
| Type | `Time Curve`  | Changes the type of the graph used to show the metric. This also affects which data of the metric the widget is focussed on. Possible choices:
...
| Coloring | _Instance Name_ | Sets the property of a metric data series that is used to select the colors the data is shown with. _Instance Name_ will show data in same color that belongs to the same instance. _Series Name_  shows the metric series in same color (independent of instance). This is individual to each widget. _Result Set Index_ will show data in same color that is at the same index in the server response. _Instance and Series Name_ shows same instance and series in same color. This is a global. This means another widget using this coloring with same instance and series will have the same color as well.
...
Use the color input to individually override the widget's color for waterline, alarming line or critical line in graphs. Press the button right next to it which shows the default color as configured in the _Colors_ settings to reset the individual color to use the default setting instead. A color set to the default changes with the default.
...
* _Amber_: values in this range indicate an degraded state
* _Green_: values in this range indicate an healthy state
* _White_: values is this range are in no particular state (neither good or bad, maybe undecidable)
...
When an alert occured for a metric shown in a graph the background of the line causing the alert is colored in _Red_ or _Amber_ default color until the alerts is stopped.

Alerts are considered ongoing as long as the state is _Red_ or _Amber_.
If it is evaludated to _Green_ or _White_ the alert stops. Stopped alerts are no longer shown on top of line graphs.
...
Alert table show the overall state of each instance in the table's legend.

_Alerts_ settings can equally be used for line graphs to create a more restictive setting.
...
Alternatively a user may chose to enter the series key manually which allows to use a wild-card _star_ instead of either a tag value (like `jvm`) or the _MetricName_. For example:
...
Newer versions of the Monitoring Console allow to configure the colors used in graphs.
There is a global color configuration affecting all graphs.
...
|====================
| Property | Default | Description
| Scheme | - | The selected scheme is applied to set the colors palette for _Data_ as well as _Defaults_ and _Opacity_. This is only a convinient way to switch all of these between different settings. The individual properties can be changed afterwards as usual without affecting the scheme.
| Data # (Palette) | (colors of _Payara_ scheme) | The list of colors to use when coloring data series in graphs. This is the line color in line charts or the bar color in bar charts. Background will use the same color with _Opacity_. Add new colors by using the `+` button. The `-` button removes the last color of the list. If there are less colors in this palette than needed for different lines in a graph new colors are derived based on this palette. The derived colors focus on bright colors with high contrast but might not always fit well with the rest of the palette. In such case consider addng more colors to the palette.
| Defaults | (default of _Payara_ scheme) | Sets the default colors for _Waterline_, _Alaraming_ and _Critical_ as well as _Red_, _Amber_, _Green_ and _White_ state lines, backgrounds and texts. Some of these can be individually overridden in the widget's configuration for these lines.
...
==== JVM Page
The _JVM_ pages gives a general overview over the state of the JVM running Payara Server.
...
* _Watches Time_: The total time it took to collect and evaluate all watches. This time is less critical but it should usually be well below 1 second. Typically in range of two digit milliseconds.
* _Alerts_: The total number of ongoing (active) alerts.


[[monitoring-console-preset-health-checks]]
==== Health Checks Page
A page that gives an overview over the state of the server health checks.
...
* _GC_: Garbage Collection Health Check, percentage of time used to collect garbage objects during last 4 seconds
...
* _Connecton Pool_: Connection Pool Health Check, percent of connections currently used for each JDBC connection pool
