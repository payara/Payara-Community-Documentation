...
IMPORTANT: Version {{ book.mpOpenAPIVersion }} of the OpenAPI specification does not define the behaviour of implementations where multiple applications are deployed to the same container. This limitation will be addressed in a later version of the Metrics specification.

[[endpoint]]
== REST Endpoint
MicroProfile OpenAPI specifies that all metrics are published at the endpoint `/openapi`. For both Payara Server and Payara Micro, this means that the endpoint can be found on any HTTP port (e.g. `8080`). The OpenAPI document displayed will always be for the most recently deployed application, except in the special case of disabling the service (see <<configuration-note, configuration>>).
