...
Provided version of the API: {{ book.mpOpenAPISpecUrl }}[MicroProfile OpenAPI {{ book.mpOpenAPIVersion }}]
...
{{ book.mpOpenAPISpecUrl }}[MicroProfile OpenAPI 1.0] was released in MicroProfile 1.3.

IMPORTANT: Version {{ book.mpOpenAPIVersion }} of the OpenAPI specification does not define the behaviour of implementations where multiple applications are deployed to the same container. This limitation will be addressed in a later version of the OpenAPI specification.
...
The OpenAPI can be configured through the link:config.adoc[Config API]. For example, these configuration properties can be put in the `microprofile-config.properties`.
...
The MicroProfile OpenAPI API provides many annotations to use to augment the OpenAPI document. These are detailed in the link:https://github.com/eclipse/microprofile-open-api/blob/master/spec/src/main/asciidoc/microprofile-openapi-spec.adoc#annotations[OpenAPI Specification]. These annotations are applied before the OASFilter.
...
image:/images/microprofile/openapi.png[Set OpenAPI Configuration]
...
|`virtualservers`
|If defined, the '/openapi' endpoint assigned to the specified virtual servers, Otherwise endpoint assigned to all virtual servers. The virtual server list must be specified as a string with comma separated IDs (names) of virtual servers.
|-
|No
|===
