...
On per-client basis, or via Microprofile Config properties following aspects of SSL communication of the client can be set:

* Trust store, listing trusted certificates and authorities -- by default the one of the server is used
* Hostname verifier, useful for suppresing trust checks for some host names
* Keystore for setting up mutual SSL trust

Programmatically it is also possible to provide implementation of `SslContext` to control all aspects of SSL communication