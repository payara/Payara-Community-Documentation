...
IMPORTANT: Version 1.0 of the MicroProfile JWT Authentication specification does not define any means for preemptive authentication, that is authenticating when the target resource (such as a JAX-RS end-point) is not protected / constrained by a security role. This practically means that if a JWT token is sent alongside a request to a public / unchecked resource, MicroProfile JWT implementations will ignore it. This limitation will be addressed in a later version of the JWT specification.
...
Payara Server and Micro since versions *4.1.2.181* and *5.181* support `@RolesAllowed` out of the box as explained above.

In same cases this may clash with existing code that interprets the same annotation using custom code.

In Payara Server and Micro *5.181* Payara's interpretation of `@RolesAllowed` for JAX-RS resources can be switched of by setting the `<jaxrs-roles-allowed-enabled>` tag in `WEB-INF/glassfish-web.xml` of a war archive to `false`. E.g. 
