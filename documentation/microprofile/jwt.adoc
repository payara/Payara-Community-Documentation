...
Authentication service (like https://auth0.com/docs/scopes/current/custom-claims[auth0], https://connect2id.com/products/server/docs/config/claims-mapping[connect2id]) offers the possibility to add custom claims to JWT tokens
but it also enforce a namespaced format to avoid possible collisions with standard claims.
...
==== *custom.namespace*
When `custom.namespace` property is set, It always take precedence over the default namespace.
