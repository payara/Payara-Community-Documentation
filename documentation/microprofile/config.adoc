...
the domain.xml property values will apply to all instances that use the
configuration
...
IMPORTANT:A `--source` value of `jndi` is not supported for this command in 
4.1.2.173. The `ConfigSource` does exist and is usable though.
