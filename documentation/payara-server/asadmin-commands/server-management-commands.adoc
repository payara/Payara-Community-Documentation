...
|====
|Option |Type |Description |Default |Mandatory
|`--sync` | none, normal or true |Specifies whether the instance is to be synced with the DAS. In normal mode only config files and top-level application file are synched. In full all files are synced. | normal |No
|`--debug` |Boolean |Specifies whether the domain is started in debug mode |false |No
|`--terse` |Boolean |Specifies whether information on the result is outputted |false |No
