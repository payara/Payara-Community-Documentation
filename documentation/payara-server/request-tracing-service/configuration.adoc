...
**Trace Store Size**:: The maximum number of traces to be stored in the local rolling trace store. A store shared in a cluster uses the maximim setting of this property of any instance in the cluster with request tracing enabled.
...
historical storing is enabled. The default value is **20** records. A store shared in a cluster uses the maximim setting of this property of any instance in the cluster with request tracing enabled.
