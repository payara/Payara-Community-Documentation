...
That means it will not override an environment variable already present but is used in case it is not defined when `InitialContextFactory.getInitialContext` is invoked.
...
When a client adapter is passed in factory's environment, every JNDI lookup is first passed to the adapter instance.
If adapter returns non-empty Optional, that return value is passed back to the client.

=== Composing multiple adapters

Client side emulation might require stubbing diverse JNDI names and return types, and handling all of that in single ClientAdapter implementation would result in not very maintenable code.
...
* `CompositeClientAdapter` concatenates multiple client adapter instances, calling adapters in defined order and returning first non-empty proxy provided by the adapters.
...
This places limits on types of objects that can be transmitted as method argument or return types.
