...
The Maven repository definition is present in the pom.xml file of the `ejb-http-client` artifact. However, when you are dependencies are managed by a Company Nexus, for example, you might need to update it to refer to the Payara Patched Project repository.

Also, have a look at the BOM feature so that versions can be defined more consistently.
