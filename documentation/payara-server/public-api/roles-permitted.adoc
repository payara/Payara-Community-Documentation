...
_Since Payara Server & Micro 4.1.2.182 and 5.182_

The Payara Public API provides a `@RolesPermitted` annotation that creates an authorisation interceptor for usage with CDI beans. This largely works in the same way as the `@RolesAllowed` common annotation, which in Java/Jakarta EE is primarily used by the EJB specification.
...
_Since Payara Server & Micro 5.193_
