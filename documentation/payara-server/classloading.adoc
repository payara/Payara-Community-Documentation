...
In order to make the server to use the bundled JSF implementation within the application, you need to set an additional configuration parameter.
You need to indicate within the _glassfish-web.xml_ file that the server should use the bundled JSF implementation as follows:
...
By specifying both options, disable class loading delegation and useBundledJsf, the bundled JSF implementation within your deployment (WAR or EAR) will be used correctly.
