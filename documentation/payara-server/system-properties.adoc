...
| fish.payara.SO_KEEPALIVE | String | Enables keep alive (`SO_KEEPALIVE`) on an IIOP Listener. Can be set as a global system property, or as a property on a specific listener (with the latter taking precedence over the global setting if both are set). Replaces deprecated property `fish.payara.SOKeepAlive`. | true, false | false (undefined)
|=======================================================================
