...
https://github.com/eclipse/microprofile-config/blob/master/Overview.adoc[the
...
an application. Property values will only be available for the specified module
...
Config cache duration can be configured by using Asadmin commands. By default properties have a TTL (time to live) of 60 seconds.
That means each individual property does not change for 60 seconds since it has been resolved before.
Therefore it can take up to 60 seconds for changes made visible by a config source to become effective.
