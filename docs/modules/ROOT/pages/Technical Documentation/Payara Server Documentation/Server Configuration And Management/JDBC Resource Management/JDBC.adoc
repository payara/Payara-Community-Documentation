...
A connection pool is a store of database connections that can be used and (more importantly)
...
SQL calls made through a JDBC connection pool can be traced,
...
then select the _JDBC > JDBC Connection pools > ${Your Connection pool}_.
...
In Java EE 7+ applications and above, a JDBC data source can be deployed by adding the
...
Payara Serverb supports setting advanced JDBC connection pool
