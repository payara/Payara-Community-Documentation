...
WARNING: When auto-naming is enabled, Payara Server will only attempt to resolve conflicts with
...
If the Docker containers are stopped externally to Payara (i.e. by the `docker` command), the node
...
The environment property is unique in that the colon character is used to denote the equals sign, as Payara Server does not
