...
If using an external RDBMS engine for storing persistent timer data is not an option, it is also possible to use the Domain Data Grid to function as a replacement in production environments. The same feature is available in xref://Technical Documentation/Payara Micro Documentation/persistent-ejb-timers.adoc[Payara Micro].
