...
Hazelcast must be enabled for all Payara Server instances and they must be able to connect to the same Hazelcast cluster. See the xref:Technical Documentation/Payara Server Documentation/hazelcast/Overview.adoc[Data Grid] section for more details.
...
by if not configured (see the section xref://Technical Documentation/Payara Micro Documentation/configuring/instance-names.adoc[Payara Micro Instance
Names]).
