...
NOTE: If both an annotation attribute and a MicroProfile Configuration property are defined for the same option then the MicroProfile Configuration property value always takes precedence over the `@OpenIdAuthenticationDefinition` annotation value.
...
Keycloak is an Open Source Identity and Access Management Server, which is a OAuth2 and OpenID Connect(OIDC) protocol complaint. In this section, the basic steps 
are described to setup Keycloak OpenId provider.
