...
CAUTION: When using MicroProfile Config properties, if you use a reference to a variable provided by Payara using the `${}` notation inside the values, you need to escape the `$` sign with a double back-slash (`\\`). Otherwise they will be treated as references to other MicroProfile Config properties, which may result in an error when no matching variables are found. This especially applies to the `baseURL` variable, which is only accepted in the value of the `redirectURI` option. For example, to specify `redirectURI` using a MicroProfile Config property which is equal to `${baseURL}/Callback` used in the annotation, you need to use the following MicroProfile configuration variable `key=value` definition: `payara.security.openid.redirectURI=\\${baseURL}/Callback`.
