...
|`/health`
|Readiness + Liveness + Backward compatible
|On invocation, all registered health checks annotated with `@Liveness`, `@Readiness` and `@Health` are performed and the results aggregated.
|====
...
â†’ [instance-configuration (like server-config)] â†’ MicroProfile â†’ Health Check:
...
[[backward-compatible]]
=== Backward compatible
Since MicroProfile Health Check 2.0, `@Health` annotation is deprecated 
and JSON payload elements `outcome` and `state` in endpoint response are replaced by `status`.
By default for backward compatibility, the deprecated `/health` endpoint invokes 
all registered health checks having `@Health`, `@Liveness` or `@Readiness` annotation.
And to enable the backward compatibility in response JSON format, set the value of 
`MP_HEALTH_BACKWARD_COMPATIBILITY_ENABLED` MicroProfile config property to true.