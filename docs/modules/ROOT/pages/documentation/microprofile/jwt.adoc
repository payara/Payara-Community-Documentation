...
By default, the public key retrieved by the `mp.jwt.verify.publickey.location` configuration property will be cached in memory for 5 minutes after being read from either a local file or a remote location. You can modify this _"time-to-live"_ that the key stays cached in memory in the case you are dealing with long-lived keys by setting the `publicKey.cache.ttl` custom property.
