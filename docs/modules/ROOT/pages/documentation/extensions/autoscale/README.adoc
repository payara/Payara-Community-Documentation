...
You can find the code for each AutoScale extension here: https://github.com/payara/AutoScale-Groups. The codebase includes a notifier example in case a notifier you want is not available and you'd like to implement it yourself.
...
TIP: Make sure that, if the AutoScale extension has dependencies in order to run, the dependency artifacts are also in
