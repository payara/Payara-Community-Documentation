...
NOTE: It is always recommended to specify the tag name of the Docker image that you plan to use. Otherwise, you will be running the latest version and might encounter some unwanted effects due to changes between versions.
...
_Since Payara Server's Docker Image 5.2020.3_
...
If preboot command files are not enough, you can add your own customized shell scripts to the `${SCRIPT_DIR}` directory, which will allow you to modify the environment before the Payara Server domain starts. 

Scripts added to this folder should be named with the following convention:

----
init_<num>_<text>.sh 
----
* `<num>` is a positive integer that allows you to set the order in which the scripts are run.
* `<text>` is a descriptive help text that summarizes what the script does.

NOTE: Be aware that the default deploy command script (`init_1_generate_deploy_commands.sh`) also uses follows this convention, so custom scripts should start with number 2.

As with the preboot command file, you may either create a custom Docker image that already contains the script files in question or you can also mount a volume mapped to the `${SCRIPT_DIR}` directory instead.
...
. `${SCRIPT_DIR}/init_*.sh`: As described above, these scripts can be provided by you to run and configure the environment *before* domain startup.
