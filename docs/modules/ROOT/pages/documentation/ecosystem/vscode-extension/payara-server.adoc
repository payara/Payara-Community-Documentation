...
Alternatively application can be deployed from *Java Projects* interface:
...
The Hot Deploy mode can be enabled in the Visual Studio Code from the *Deployment Settings* of the Payara Server.
...
After enabling the Hot Deploy settings as mentioned above, Simply deploy the Web Applications to Payara Server Community
...
can focus on the key development task.