...
_User_s and _Guest_s pull the server configuration to update their local configuration with the base.
...
_Administrator_s also find a button to _Update Remote Pages_ with any changes applied to local changes.
...
_User_s and _Administrator_s can both _Push_ a page to update the persistent remote configuration and _Pull_ the remote configuration to update the local page.
_Administrator_s can also enable or disable the _auto_matic page synchronisation.
...
When no role is set for the session the user is asked to chose the role when opening monitoring console.
...
For manual sharing the configuration can be exported and imported in the form of JSON files to share or distribute configurations.
Server integrated sharing can be done using the build in page synchronisation which is a two way update mechanism to update either the local configuration with server configuration or the other way around which allows to share changes made by one user with other users in a more convenient way.
