...
The _Reset_ button can be used to force a page update before the next interval starts and ends.
Changes to the _Query Series_, _Query Interval_ or _Max Size_ do not have immediate effect and can equally applied immediately using the _Reset_ button.
...
|====================
| Property | Default | Description
| Series | - | The key(s) of the data series shown in the graph. Use `+` button to add further series to the same graph. To remove one series or multiple ones blank out the input field containing the series to remove. Note that only series of the same unit (same Y-axis) are meant to be shown in the same graph. Multiple series of different Y-axis are currently not supported.
