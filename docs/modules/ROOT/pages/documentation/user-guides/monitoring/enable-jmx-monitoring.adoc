...
"Configure Monitoring" xref for the relevant server instance. In the image below
we only have the DAS ("`server`") to choose:

image:guides/monitoring/admin-console-monitoring1.png[Click the xref for the relevant instance]

That xref will take you directly to the correct page for the `server-config` -
