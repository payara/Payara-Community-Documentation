...
By default, Payara checks if the submitted Client Certificate is present in the trust store and the certificate is valid time wise. If it is found and valid, the Client Certificate accepted without any additional checks.
