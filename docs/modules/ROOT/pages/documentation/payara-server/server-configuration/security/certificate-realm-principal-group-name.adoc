[[certificate-realm-principal-name-group-names]]
= Certificate Realm: Principal Name, Group Names

[[configure-the-principal-name-on-certificate-realms]]
== Configure the Principal Name on Certificate Realms

Payara Server Community includes a feature that supports using exclusively the use of the Common-Name (`CN`) from a client
...
xref:documentation//payara-server/server-configuration/security/certificate-realm-principal-group-name.adoc[using CN as principal name],
