...
_Since Payara Server 5.2021.1_
...
IMPORTANT: The command will only clean jobs that are stored in the internal embedded H2 database. If a separate database is used to store the records for JBatch jobs, the command will not clear them. Support for other database storage types will come later in the future
...
[source, shell]
----
asadmin > clean-jbatch-repository --status=ALL --days=7 test job
----
